            ...
            <head>
                ...
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    h1 { color: #dc2626; font-size: 1.75rem; margin-bottom: 1rem; }
...
// ============================================================
// ATTENDANCE SYSTEM - Normal code below (unchanged)
// ============================================================
...
class AttendanceSystem {
        ...
        if (this.currentView === 'by-employee') {
            this.statsContent.innerHTML = `
                <div class="stats-grid">
                    <div class="stat-item"><div class="stat-value">${stats.totalDays}</div><div class="stat-label">Jours total</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.presentDays}</div><div class="stat-label">Présences</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.lateDays}</div><div class="stat-label">Retards</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.totalHours}h</div><div class="stat-label">Heures totales</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.averageHours}h</div><div class="stat-label">Moyenne/jour</div></div>
                </div>`;
        } else if (this.currentView === 'today' || this.currentView === 'by-date') {
            this.statsContent.innerHTML = `
                <div class="stats-grid">
                    <div class="stat-item"><div class="stat-value">${stats.total || stats.totalEmployees || 0}</div><div class="stat-label">Employés total</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.present || stats.presentToday || 0}</div><div class="stat-label">Présents</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.late || stats.lateToday || 0}</div><div class="stat-label">Retards</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.absent || stats.absentToday || 0}</div><div class="stat-label">Absents</div></div>
                    <div class="stat-item"><div class="stat-value">${stats.inProgress || stats.inProgressToday || 0}</div><div class="stat-label">En cours</div></div>
                </div>`;
        }
    }
        ...
        pageData.forEach(record => {
            ...
            tableHTML += `
                <tr onclick="attendanceSystem.showDetails('${record.uid}', '${record.date}')" style="cursor: pointer; transition: background-color 0.2s;">
                    ...
                    <td><div style="font-weight: 600; color: #111827;">${record.name || 'Non renseigné'}</div></td>
                    <td>
                    ...
                    <td>${record.arrivalTime ? `<div class="time-display arrival-time">${record.arrivalTime}</div>` : '<div class="time-display empty-time">-</div>'}</td>
                    <td>${record.departureTime ? `<div class="time-display departure-time">${record.departureTime}</div>` : `<div class="time-display empty-time ${isToday ? 'pending-time' : ''}">${isToday ? 'En attente' : '-'}</div>`}</td>
                    <td><div class="hours-display" style="color: ${hoursColor}">${record.hoursWorked > 0 ? `${record.hoursWorked}h` : (isToday && record.arrivalTime ? 'En cours...' : '-')}</div></td>
    ...
    async showDetails(uid, date) {
        try {
            ...
            const data = await response.json();
            if (!data.success) throw new Error('Impossible de charger les détails');
            ...
            let modalHTML = `
                <div class="employee-info-section">
                    ...
                    <div class="info-grid">
                        <div class="info-item"><div class="info-label">Matricule</div><div class="info-value">${employee.cardNo || 'N/A'}</div></div>
                        <div class="info-item"><div class="info-label">ID Employé</div><div class="info-value">${employee.userId || 'N/A'}</div></div>
                        <div class="info-item"><div class="info-label">Nom complet</div><div class="info-value">${employee.name || 'Non renseigné'}</div></div>
                        <div class="info-item"><div class="info-label">Jour</div><div class="info-value">${record.dayName || this.getDayName(date)}</div></div>
                    </div>
                </div>
                <div class="attendance-section">
                    ...
                    <div class="attendance-grid">
                        ...
                        <div class="attendance-card hours-card">
                            ...
                            <div class="card-content"><div class="card-label">Heures travaillées</div><div class="card-value">${record.hoursWorked > 0 ? `${record.hoursWorked}h` : (isToday && record.arrivalTime ? 'En cours...' : '-')}</div></div>
                        </div>
            ...
            if (record.entries && record.entries.length > 0) {
                ...
                modalHTML += `
                    <div class="entries-section">
                        ...
                        <div class="no-entries"><i class="fas fa-times-circle"></i><p>Aucun pointage enregistré pour ce jour</p></div>
                    </div>`;
            }
                ...
                modalHTML += `
                    <div class="debug-section">
                        ...
                        <div class="debug-grid">
                            <div class="debug-item"><span class="debug-label">UID:</span><span class="debug-value">${record.uid}</span></div>
                            <div class="debug-item"><span class="debug-label">Pointeuse User ID:</span><span class="debug-value">${record.pointeuseUserId}</span></div>
                            <div class="debug-item"><span class="debug-label">Date traitement:</span><span class="debug-value">${new Date().toLocaleString('fr-FR')}</span></div>
                        </div>
                    </div>`;
            }
    ...
    showNotification(message, type = 'info') {
        ...
        notification.innerHTML = `
            <div class="notification-icon"><i class="fas fa-${type === 'success' ? 'check-circle' : type === 'error' ? 'exclamation-circle' : type === 'warning' ? 'exclamation-triangle' : 'info-circle'}"></i></div>
    ...
    async showDebugInfo() {
        try {
            ...
            if (data.success) {
                let debugHTML = `
                    <div class="debug-header"><h3><i class="fas fa-cogs"></i> Informations système</h3></div>
                    <div class="debug-stats">
                        <div class="debug-stat"><span class="stat-label">Utilisateurs:</span><span class="stat-value">${data.stats.users || 0}</span></div>
                        <div class="debug-stat"><span class="stat-label">Logs:</span><span class="stat-value">${data.stats.logs || 0}</span></div>
                        <div class="debug-stat"><span class="stat-label">Enregistrements traités:</span><span class="stat-value">${data.stats.processed || 0}</span></div>
                        <div class="debug-stat"><span class="stat-label">Taux de correspondance:</span><span class="stat-value">${data.stats.matchRate || '0%'}</span></div>
                    </div>
                    <div class="debug-section"><h4><i class="fas fa-users"></i> Correspondances employés</h4><div class="matches-list">`;
                data.sampleLogs?.forEach((item, index) => {
                    debugHTML += `
                        <div class="match-item ${item.matchedUser ? 'matched' : 'not-matched'}">
                            <div class="match-info"><strong>Log ${index + 1}:</strong> UID=${item.log.uid}, Type=${item.log.type}</div>
                            <div class="match-result">${item.matchedUser ? `<span class="success"><i class="fas fa-check"></i> ${item.matchedUser.name}</span>` : '<span class="error"><i class="fas fa-times"></i> Aucune correspondance</span>'}</div>
                        </div>`;
                });
                debugHTML += `</div></div>
                    <div class="debug-actions">
                        <button onclick="attendanceSystem.runDebugTests()" class="btn-secondary"><i class="fas fa-vial"></i> Lancer les tests</button>
                        <button onclick="attendanceSystem.forceRefresh()" class="btn-warning"><i class="fas fa-redo"></i> Forcer rafraîchissement</button>
                    </div>`;
    ...
    async runDebugTests() {
        try {
            ...
            this.debugModal.querySelector('#debug-body').innerHTML += `
    ...
    showHelp() {
        const helpHTML = `
            <div class="help-content">
                ...
                <div class="help-section"><h4><i class="fas fa-eye"></i> Modes d'affichage</h4><ul>
                ...
                <div class="help-section"><h4><i class="fas fa-filter"></i> Filtres disponibles</h4><ul>
                ...
                <div class="help-section"><h4><i class="fas fa-chart-bar"></i> Statistiques</h4><p>Cliquez sur le bouton "Stats" pour afficher/masquer les statistiques.</p></div>
                <div class="help-section"><h4><i class="fas fa-download"></i> Exportation</h4><p>Utilisez le bouton "Exporter CSV" pour télécharger les données au format Excel.</p></div>
                <div class="help-section"><h4><i class="fas fa-bug"></i> Débogage</h4><p>Utilisez le bouton "Débogage" pour diagnostiquer les problèmes de connexion.</p></div>
                <div class="help-tips"><h4><i class="fas fa-lightbulb"></i> Conseils</h4><ul>
    ...
    getAttendanceStatus(record) {
        ...
        if (arrivalTotalMinutes < 8 * 60) return 'À l\'heure';
        if (arrivalTotalMinutes <= 9 * 60) return 'Présent';
        return 'En retard';
    }
    ...
    getStatusClass(status) {
        switch (status) {
            ...
            case 'Présent (départ manquant)': return 'status-warning';
...
additionalStyles.textContent = `
    .today-badge { background: #dbeafe; color: #1e40af; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem; display: inline-block; margin-top: 4px; }
    .today-indicator { background: #3b82f6; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.85rem; margin-left: 8px; }
    .time-display { font-weight: 600; font-size: 1.1rem; padding: 4px 8px; border-radius: 6px; display: inline-block; }
    .arrival-time { color: #059669; background: rgba(16, 185, 129, 0.1); }
    .departure-time { color: #dc2626; background: rgba(220, 38, 38, 0.1); }
    ...
    .time-entry { padding: 3px 6px; border-radius: 4px; font-size: 0.85rem; font-weight: 500; display: inline-block; }
    .arrival-entry { background: #d1fae5; color: #065f46; }
    .departure-entry { background: #fee2e2; color: #991b1b; }
    .action-btn { padding: 4px 8px; background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; cursor: pointer; transition: all 0.2s; }
    .action-btn:hover { background: #e5e7eb; border-color: #9ca3af; }
    .page-number { min-width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--border-color); border-radius: 6px; cursor: pointer; transition: all 0.2s; font-size: 0.875rem; background: white; }
    .page-number:hover { background-color: #f3f4f6; border-color: #9ca3af; }
    .page-number.active { background-color: #3b82f6; color: white; border-color: #3b82f6; font-weight: 600; }
    .page-ellipsis { padding: 0 8px; color: #6b7280; }
    ...
    .info-item { background: #f8fafc; padding: 0.75rem; border-radius: 6px; }
    .info-label { font-size: 0.75rem; color: #6b7280; margin-bottom: 0.25rem; }
    .info-value { font-size: 0.875rem; font-weight: 600; color: #1f2937; }
    ...
    .entries-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 0.75rem; margin: 1rem 0; }
